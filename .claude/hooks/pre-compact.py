#!/usr/bin/env python3
"""
HIVE Pre-Compaction Hook — Snapshot state before context compression

PreCompact can ONLY do side effects (no additionalContext, no blocking).
This hook:
  1. Auto-updates CLAUDE_STATE.md ephemeral sections from transcript
  2. Saves a full snapshot to disk for post-compaction restore

Flow:
  1. PreCompact fires → this script updates CLAUDE_STATE.md + writes snapshot
  2. Compaction happens (context compressed)
  3. SessionStart fires with matcher "compact" → session-start.py reads
     the snapshot and injects it as additionalContext
"""
...
from _state_utils import auto_update_state, parse_curated_sections, STATE_PATH
...
def main():
    ...
    # Step 1: Auto-update CLAUDE_STATE.md ephemeral sections
    ...
    # Include the full (now freshly updated) CLAUDE_STATE.md in the snapshot
    if os.path.exists(STATE_PATH):
        try:
            with open(STATE_PATH, 'r', encoding='utf-8') as f:
                state_content = f.read()
            lines.append("## Claude Working State (auto-saved)")
