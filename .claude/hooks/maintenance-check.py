#!/usr/bin/env python3
"""
HIVE Maintenance Check Hook — Run before session ends
...
CODE_EXTENSIONS = {
    '.rs', '.ts', '.tsx', '.js', '.jsx', '.css', '.html', '.toml',
    '.py', '.yaml', '.yml', '.json', '.sql',
}
...
def main():
    ...
    output = {
        "decision": "block",
        "reason": (
            "HIVE MAINTENANCE CHECK\n\n"
            f"Code files were modified this session:\n{file_list}\n\n"
            "Before stopping, check if any documentation needs updating:\n\n"
            "| If this changed...       | Update...                          |\n"
            "|--------------------------|------------------------------------||\n"
            "| Features or behavior     | ROADMAP.md (feature status)|\n"
            "| Build process or setup   | README.md                          |\n"
            "| Code patterns or lessons | CLAUDE.md                          |\n"
            "| Persistent insights      | MEMORY.md (via memory dir)         |\n\n"
