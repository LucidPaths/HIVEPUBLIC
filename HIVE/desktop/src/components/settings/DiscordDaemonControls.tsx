...
import type { DiscordDaemonStatus, AccessLists } from '../../types';

export default function DiscordDaemonControls() {
  const [status, setStatus] = useState<DiscordDaemonStatus | null>(null);
  ...
  async function refreshStatus() {
    try {
      const s = await api.getDiscordDaemonStatus();
      setStatus(s);
    } catch {
  ...
  async function refreshAccessLists() {
    try {
      const lists = await api.getDiscordAccessLists();
  ...
  async function handleStart() {
    setLoading(true);
    try {
      const result = await api.startDiscordDaemon();
  ...
  async function handleStop() {
    setLoading(true);
    try {
      const result = await api.stopDiscordDaemon();
      setMsg({ text: result, ok: true });
      await refreshStatus();
    } catch (e) {
      setMsg({ text: `${e}`, ok: false });
  ...
  async function addHostId() {
    ...
    try {
      await api.setDiscordHostIds(updated);
  ...
  async function addUserId() {
    ...
    try {
      await api.setDiscordUserIds(updated);
    ...
    <div className="bg-zinc-900 rounded-lg p-4 mb-4">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          <Activity className={`w-4 h-4 ${isRunning ? 'text-indigo-400 animate-pulse' : 'text-zinc-500'}`} />
          <span className="text-white font-medium text-sm">Discord Daemon</span>
          {isRunning && status?.connected_bot && (
            <span className="text-xs text-indigo-400">{status.connected_bot}</span>
          )}
        </div>
        <div className="flex items-center gap-2">
          <button
      ...
      <p className="text-zinc-500 text-xs mb-2">
        {isRunning
          ? 'HIVE is listening for Discord messages. Messages auto-trigger the agentic loop.'
          : 'Start the daemon to receive and auto-respond to Discord messages.'}
      </p>
        ...
        <div className="mt-3 space-y-3 border-t border-zinc-700 pt-3">
          ...
          <div>
            <div className="flex items-center gap-1.5 mb-1.5">
              ...
              <span className="text-zinc-600 text-xs">— full access (your Discord user ID)</span>
            </div>
          ...
          <div>
            ...
            <div className="flex gap-1.5 mt-1.5">
              <input
                ...
                placeholder="User ID (e.g. 987654321098765432)"
          ...
          <p className="text-zinc-600 text-xs">
            Enable Developer Mode in Discord, right-click your profile, Copy User ID. Host = full tool access. User = safe tools only.
          </p>
        </div>
      )}
    </div>
